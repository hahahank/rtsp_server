...
def draw_text(frame):
    ...
    formatted_date = now.strftime("%Y-%m-%d %H:%M:%S") # format time
#    formatted_date = str(int(time.time()))  # string time
    # 在图像上方添加时间文本
    font = cv2.FONT_HERSHEY_SIMPLEX
    text_position = (10, 30)  # 文本的位置在图像左上角
    font_scale = 1
    font_color = (0, 255, 0)  # BGR颜色
    font_thickness = 2
    cv2.putText(frame, formatted_date, text_position, font, font_scale, font_color, font_thickness)
    return frame
